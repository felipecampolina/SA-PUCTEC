...
function _Images_ProcessSrc_CopyImageToHost( &$ctxProcess, $imgSrc, $imgCont, $settCache, $settImg )
{
	...
	{
		$imgSrc -> srcInfo[ 'srcWoArgs' ] = '';
		$args = Image_MakeOwnRedirUrlArgs( substr( $file, strlen( $ctxProcess[ 'siteRootPath' ] ) + 1 ) );
		$imgSrc -> src = Net::UrlDeParse( array( 'path' => $imgSrc -> srcInfo[ 'srcWoArgs' ], 'query' => $args ), Net::URLPARSE_F_PRESERVEEMPTIES );
	}
	else
...
function Images_ProcessSrcEx( &$ctxProcess, $imgSrc, $settCache, $settImg )
{
	...
	if( !$file )
	{
		...
		if( Gen::GetArrField( $settImg, array( 'inlSml' ), false ) && strlen( $imgCont ) <= Gen::GetArrField( $settImg, array( 'inlSmlSize' ), 0 ) )
			$imgSrc -> src = _Images_ProcessSrc_InlineEx( $imgSrc -> mimeType, $imgCont );
		else if( !_Images_ProcessSrc_CopyImageToHost( $ctxProcess, $imgSrc, $imgCont, $settCache, $settImg ) )
			return( false );
...
{
	return( Ui::Tag( 'svg', $fill ? Ui::TagOpen( 'rect', array( 'width' => '100%', 'height' => '100%', 'fill' => $fill ), true ) : null
		, array_merge( array( 'xmlns' => 'http://www.w3.org/2000/svg', 'viewBox' => '0 0 ' . $width . ' ' . $height ), $exact ? array( 'width' => $width, 'height' => $height ) : array() ) ) );
}

function LazyLoad_SrcSubst( $width, $height, $exact = false, $fill = null )
{
	if( !$width )
		$width = 225;
	if( !$height )
		$height = $width / 3 * 2;
	return( 'data:image/svg+xml,' . rawurlencode( LazyLoad_SvgSubst( $width, $height, $exact, $fill ) ) );
...
function _Images_ProcessItemLazy_Start( &$ctxProcess, $doc, $settImg, $item )
{
	...
	if( Gen::GetArrField( $settImg, array( 'lazy', 'del3rd' ), false ) )
	{
		...
		HtmlNd::AddRemoveAttrClass( $item, array(), array( 'lazyload', 'blog-thumb-lazy-load', 'lazy-load', 'lazy', 'mfn-lazy', 'iso-lazy-load', 'll-image', 'wd-lazy-load', 'wd-lazy-fade', 'houzez-lazyload' ) );
...
{
	if( !( Gen::GetArrField( $settImg, array( 'srcAddLm' ), false ) || Gen::GetArrField( $settImg, array( 'inlSml' ), false ) || Gen::GetArrField( $settImg, array( 'deinlLrg' ), false ) || Gen::GetArrField( $settImg, array( 'lazy', 'setSize' ), false ) || Gen::GetArrField( $settImg, array( 'lazy', 'load' ), false ) || Gen::GetArrField( $settCdn, array( 'enable' ), false ) ) )
		return( true );
	...
	{
		if( !HtmlNd::FindUpByTag( $itemSvgImg, 'svg' ) )
			continue;
