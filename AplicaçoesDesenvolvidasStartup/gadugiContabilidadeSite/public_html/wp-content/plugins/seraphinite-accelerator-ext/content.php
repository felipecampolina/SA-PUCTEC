...
function OnEarlyContentComplete( $buffer, $tmpUpdate = false )
{
	...
	{
		if( !headers_sent() )
			header( 'X-Seraph-Accel-Content: 2.20.11; status=' . $skipStatus . ', procTime=' . $tmProc . 's' );
		...
		{
			{
...

				{
					...
					if( apply_filters( 'seraph_accel_jscss_addtype', false ) )
						$item -> setAttribute( 'type', 'text/css' );
					$item -> nodeValue = htmlspecialchars( ( Gen::GetArrField( $settImg, array( 'lazy', 'smoothAppear' ), false ) ? 'img.lzl,img.lzl-ing{opacity:0.01;}img.lzl-ed{transition:opacity .25s ease-in-out;}' : '' ) . '.lzl{background-image:none!important;}' . ( $bLazyCont ? 'i[data-lzl-nos]{height:10em;display:block}' : '' ) );
