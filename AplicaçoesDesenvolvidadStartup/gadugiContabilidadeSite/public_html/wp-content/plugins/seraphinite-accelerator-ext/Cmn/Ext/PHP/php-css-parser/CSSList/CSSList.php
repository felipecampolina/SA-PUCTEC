...
abstract class CSSList extends Renderable implements Commentable {
	...
	public static function parseList(ParserState $oParserState, CSSList $oList) {
		...
		if(!$bIsRoot && !($bLenientParsing & Settings::ParseErrHigh)) {
			throw new SourceException(Settings::ParseErrHigh, "Unexpected end of document", $oParserState->currentPos());
		}
	}
		...
		if ($oParserState->comes('@')) {
			...
			if ($bIsRoot) {
				if ($oParserState->getSettings()->bLenientParsing & Settings::ParseErrHigh) {
					...
					throw new SourceException(Settings::ParseErrHigh, "Unopened {", $oParserState->currentPos());
				}
			} else {
				return null;
			}
	...
	private static function parseAtRule(ParserState $oParserState) {
		...
		if ($sIdentifier === 'import') {
			...
			if (substr_count($sArgs, '(') != substr_count($sArgs, ')')) {
				if (!($oParserState->getSettings()->bLenientParsing & Settings::ParseErrHigh)) {
					throw new SourceException(Settings::ParseErrHigh, "Unmatched brace count in media query", $oParserState->currentPos());
				}
				$sArgs = '';
			}
			$bUseRuleSet = true;
