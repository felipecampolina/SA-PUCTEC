...
class Gen
{
	...
	static function FileOpenWithMakeDir( &$h, $filename, $mode, $use_include_path = false )
	{
		...
		if( @file_exists( $dir ) )
		{
			...
			if( !( Gen::StrStartsWith( $mode, 'w' ) && @is_dir( $filename ) ) )
				return( Gen::E_FAIL );
	...
	static function LogClear( $file )
	{
		@file_put_contents( $file, '' );

		foreach( @glob( Gen::GetFileName( $file, true, true ) . '.*.' . Gen::GetFileExt( $file ), GLOB_NOSORT ) as $filePrev )
			@unlink( $filePrev );
	}
...
class Lock
{
	...
	function Acquire( $wait = true )
	{
		...
		if( @flock( $this -> h, LOCK_NB | LOCK_EX ) )
		{
			$this -> mode |= 2;
			return( true );
		}
...
class ArrayOnFiles implements \Iterator, \ArrayAccess, \Countable
{
	...
	public function __construct( $dirFilesPattern , $options = null )
	{
		...
		$this -> options = array_merge( array( 'countPerChunk' => 1000, 'countSep' => '@', 'compr' => 'gz', 'keys' => true , 'cbSort' => null ), ( array )$options );
		$this -> iIterChunk = -2;
		$this -> iChunk = 0;
		$this -> aChunk = array();
...
class Net
{
	...
	static function GetRequestHeaders( $serverArgs = null )
	{
		if( $serverArgs === null )
			$serverArgs = $_SERVER;
...
class HtmlNd
{
	...
	static function InsertAfter( $nd, $ndChild, $ndChildAfter )
	{
		if( !$nd || !$ndChild )
			return( false );

		$nd -> insertBefore( $ndChild, $ndChildAfter ? $ndChildAfter -> nextSibling : null );
		return( true );
	}
...
class Wp
{
	...
	static private function _RemoteGet_Ctx()
	{
		$obj = new AnyObj();
	...
	static function RemoteGet( $url, $args = null )
	{
		...
		$obj = self::_RemoteGet_Ctx();

		$obj -> setHooks( true );
		$res = wp_remote_get( $url, $args );
		$obj -> setHooks( false );

		return( $obj -> adjustRes( $res ) );
	}
