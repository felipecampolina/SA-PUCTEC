...
if (!function_exists('upvf_classic_display_prvt_files')) {
	function upvf_classic_display_prvt_files($atts){
		if(is_user_logged_in()){
			...
			$grp_by = 0;
			$grp_by_checked = '';
			if( isset($_GET['grp_by']) && isset($_GET['_wpnonce']) ){
				if( wp_verify_nonce($_GET['_wpnonce'], 'upf_grp_nonce') ){
			...
			foreach($all_docs_ids as $doc_id){
				...
				if($grp_by){ // group together same type of files
					if (strpos($mime_type, 'image') !== false) {
						$grp_img_html .= '<div id="doc_'.$doc_id.'" class="doc-item" data-alwd-usrs="'.$ca_users_str.'">';
						...
						$grp_doc_html .= '<div id="doc_'.$doc_id.'" class="doc-item" data-alwd-usrs="'.$ca_users_str.'">';
					...
					$doc_item_html .= '<div id="doc_'.$doc_id.'" class="doc-item" data-alwd-usrs="'.$ca_users_str.'">';
					if (strpos($mime_type, 'image') !== false) {
			...
			if($all_shared_docs){
				...
				foreach($all_shared_docs as $swm_doc){
					...
					$swm_item_html .= '<div id="swm_doc_'.$swm_doc.'" class="swm-doc-item">';
